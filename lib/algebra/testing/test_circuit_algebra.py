...
class TestCircuitAlgebra(unittest.TestCase):
    ...
    def testSeries(self):
        ...
        self.assertEquals( A<< B, SeriesProduct.apply_with_rules(A,B))
        
        # need at least two operands
        self.assertRaises(Exception, SeriesProduct, ())
        self.assertRaises(Exception, SeriesProduct.apply_with_rules, ())
        self.assertRaises(Exception, SeriesProduct, (A,))
        
        self.assertEquals(SeriesProduct.apply_with_rules(A), A)
    ...
    def testSeriesFilterIdentities(self):
        for n in (1,2,3, 10):
            ...
            self.assertEquals(SeriesProduct.apply_with_rules(idn, idn, A, idn, idn, B, idn, idn), A << B)
    ...
    def testConcatenation(self):
        ...
        self.assertEquals(A+B, Concatenation.apply_with_rules(A,B))
    ...
    def testDistributiveLaw(self):
         ...
         self.assertEquals(SeriesProduct.apply_with_rules((A+B), (C+D+E), (A+B)), Concatenation(A << (C+D)<< A,  B << E<< B))
    ...
    def testFeedback(self):
        ...
        self.assertRaises(Exception, Feedback.apply_with_rules, (circuit_identity(0)))
        self.assertEquals(Feedback.apply_with_rules(circuit_identity(n)), circuit_identity(n-1))
        self.assertEquals(Feedback.apply_with_rules(A+B), A + Feedback.apply_with_rules(B))
