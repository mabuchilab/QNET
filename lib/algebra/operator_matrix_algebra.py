...
class OpOpMatrixProduct(MatrixOperation, CoefficientTermProduct):
    ...
    def apply_with_rules(cls, coeff, term):
        ...
        if isinstance(term, OpOpMatrixProduct):
            return OpOpMatrixProduct.apply_with_rules(coeff * term.coeff, term.term)
...
OperatorMatrix.add_map[OperatorMatrix] = MatrixAddition.apply_with_rules
...
def rmul_by_non_matrix(matrix, other):
    if not isinstance(other, Operator):
        other = other * IdentityOperator()
    return OpOpMatrixProduct.apply_with_rules(other, matrix)
...
OperatorMatrix.mul_map[OperatorMatrix] = MatrixMultiplication.apply_with_rules
