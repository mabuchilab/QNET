...
from qnet.printing.latexmod import latex as tex, QnetLatexPrinter


def test_tex_render_string():
    """Test rendering of ascii to latex strings"""
    assert QnetLatexPrinter.render_string('a') == r'a'
    assert QnetLatexPrinter.render_string('A') == r'A'
    assert QnetLatexPrinter.render_string('longword') == r'\text{longword}'
    assert QnetLatexPrinter.render_string('alpha') == r'\alpha'
    assert QnetLatexPrinter.render_string('Alpha') == r'A'
    assert QnetLatexPrinter.render_string('Beta') == r'B'
    assert QnetLatexPrinter.render_string('Gamma') == r'\Gamma'
    assert QnetLatexPrinter.render_string('Delta') == r'\Delta'
    assert QnetLatexPrinter.render_string('Epsilon') == r'E'
    assert QnetLatexPrinter.render_string('Zeta') == r'Z'
    assert QnetLatexPrinter.render_string('Eta') == r'H'
    assert QnetLatexPrinter.render_string('Theta') == r'\Theta'
    assert QnetLatexPrinter.render_string('Iota') == r'I'
    assert QnetLatexPrinter.render_string('Kappa') == r'K'
    assert QnetLatexPrinter.render_string('Lambda') == r'\Lambda'
    assert QnetLatexPrinter.render_string('Mu') == r'M'
    assert QnetLatexPrinter.render_string('Nu') == r'N'
    assert QnetLatexPrinter.render_string('Xi') == r'\Xi'
    assert QnetLatexPrinter.render_string('Omicron') == r'O'
    assert QnetLatexPrinter.render_string('Pi') == r'\Pi'
    assert QnetLatexPrinter.render_string('Rho') == r'P'
    assert QnetLatexPrinter.render_string('Sigma') == r'\Sigma'
    assert QnetLatexPrinter.render_string('Tau') == r'T'
    assert QnetLatexPrinter.render_string('Ypsilon') == r'\Upsilon'
    assert QnetLatexPrinter.render_string('Upsilon') == r'\Upsilon'
    assert QnetLatexPrinter.render_string('ypsilon') == r'\upsilon'
    assert QnetLatexPrinter.render_string('upsilon') == r'\upsilon'
    assert QnetLatexPrinter.render_string('Phi') == r'\Phi'
    assert QnetLatexPrinter.render_string('Chi') == r'X'
    assert QnetLatexPrinter.render_string('Psi') == r'\Psi'
    assert QnetLatexPrinter.render_string('Omega') == r'\Omega'
    assert QnetLatexPrinter.render_string('xi_1') == r'\xi_{1}'
    assert QnetLatexPrinter.render_string('xi_1^2') == r'\xi^{2}_{1}'
    assert QnetLatexPrinter.render_string('Xi_1') == r'\Xi_{1}'
    assert QnetLatexPrinter.render_string('Xi_long') == r'\Xi_{\text{long}}'
    assert QnetLatexPrinter.render_string('Xi_1+2') == r'\Xi_{1+2}'
    assert QnetLatexPrinter.render_string('Lambda_i,j') == r'\Lambda_{i,j}'
    assert QnetLatexPrinter.render_string('epsilon_mu,nu') == r'\epsilon_{\mu,\nu}'
