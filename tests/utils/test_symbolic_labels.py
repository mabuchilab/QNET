...
from qnet import IdxSym, FockIndex, StrLabel, SpinIndex, symbols
...
def test_evaluate_symbolic_labels():
    ...
    lbl = SpinIndex(i+j)
    assert lbl.substitute({i: 1, j: 2}) == '+3'
    assert lbl.substitute({i: 1}) == SpinIndex(1+j)
    assert lbl.substitute({j: 2}) == SpinIndex(i+2)
    assert lbl.substitute({i: 1}).substitute({j: 2}) == '+3'
    assert lbl.substitute({}) == lbl

    lbl = SpinIndex((i+j)/2)
    assert lbl.substitute({i: 1, j: 2}) == '+3/2'
    assert lbl.substitute({i: 1}) == SpinIndex((1+j)/2)
    assert lbl.substitute({j: 2}) == SpinIndex((i+2)/2)
