...
class Operator(Algebra):
    ...
    def extend_to_space(self, space):
        ...
        return OperatorTimes.create(self, *[explicit_identity_operator(s) for s in missing_spaces])
...
def extend_to_space(obj, space):
    if isinstance(obj, Operator):
        ...
        return obj * OperatorTimes.create(*[explicit_identity_operator(s) for s in space])
    else:
        ...
        if isinstance(lhs, LocalOperator):
            if lhs.space == rhs.space:
                ...
                if isinstance(rhs, OperatorPlus) and rhs.space == lhs.space:
                    return coeff * OperatorPlus.create(*[lhs*rhs_s for rhs_s in rhs.operands])
        elif isinstance(rhs, LocalOperator):
            if lhs.space == rhs.space:
                if isinstance(lhs, OperatorPlus):
                    return coeff * OperatorPlus.create(*[lhs_s * rhs for lhs_s in lhs.operands])
        elif lhs.space == rhs.space and len(lhs.space) == 1:
            if isinstance(lhs, OperatorPlus) and isinstance(rhs, OperatorPlus):
                return coeff * OperatorPlus.create(*[lhs_s*rhs_s for lhs_s in lhs.operands for rhs_s in rhs.operands])
        if coeff != 1:
...
scalar_as_operator = lambda n: (ScalarTimesOperator.create(n, IdentityOperator()))
...
Operator.add_map[Operator] = OperatorPlus.create
# Operator.add_map[Scalar] = second_arg_as_operator(OperatorPlus.apply_with_rules)
Operator.add_map[SympyBasic] = second_arg_as_operator(OperatorPlus.create)
Operator.add_map[Number] = second_arg_as_operator(OperatorPlus.create)
...
Operator.mul_map[Operator] = OperatorTimes.create

# Operator.mul_map[Scalar] = reverse_args(ScalarTimesOperator.apply_with_rules)
Operator.mul_map[Number] = reverse_args(ScalarTimesOperator.create)
...
Operator.mul_map[SympyBasic] = reverse_args(ScalarTimesOperator.create)

# Operator.rmul_map[Scalar] = reverse_args(ScalarTimesOperator.apply_with_rules)
Operator.rmul_map[Number] = reverse_args(ScalarTimesOperator.create)
# Operator.rmul_map[Number] = lambda op, number: op * sympify_if_not_pure_complex(number)
Operator.rmul_map[SympyBasic] = reverse_args(ScalarTimesOperator.create)
...
def expand_rhs(cls, operands):
    if len(operands) == 2:
        ...
        if isinstance(rhs, Addition):
            return (rhs.__class__.operation(*tuple((cls.create(lhs,rhs_summand) for rhs_summand in rhs.operands))),)
