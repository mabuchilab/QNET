    ...
    def simplify_binary(cls, lhs, rhs, **rules):
            ...
            if rules['combine_distributively']:
                if not isinstance(rhs, CPermutation) \
                    ...
                    if len(res_struct) > 1:
                        ...
                        parallel_series = [cls.create(lb, rb, **rules)  for (lb, rb) in izip(blocks, oblocks)]
                        return Concatenation.create(*parallel_series, **rules)
        ...
        if rules['factor_permutations_for_blocks']:
            ...
            if isinstance(lhs, CPermutation) and len(rbs) > 1:
                ...
                return cls.create(residual_lhs, transformed_rhs, carried_through_lhs, **rules)
    ...
    def simplify_binary(cls, lhs, rhs, **rules):
        ...
        if isinstance(lhs, SeriesProduct) and isinstance(lhs.operands[-1], CPermutation):
            if isinstance(rhs, SeriesProduct) and isinstance(rhs.operands[-1], CPermutation):
                left_part =  cls.create(SeriesProduct.create(*lhs.operands[:-1],**rules), SeriesProduct.create(*rhs.operands[:-1],**rules),**rules)
                right_part = cls.create(lhs.operands[-1], rhs.operands[-1], **rules)
                factored_out =  SeriesProduct.create(left_part, right_part, **rules)
                assert isinstance(factored_out, SeriesProduct)
                return factored_out
            else:
                left_part =  cls.create(SeriesProduct.create(*lhs.operands[:-1],**rules), rhs, **rules)
                right_part = cls.create(lhs.operands[-1], cid(rhs.cdim), **rules)
                factored_out =  SeriesProduct.create(left_part, right_part, **rules)
                assert isinstance(factored_out, SeriesProduct)
                return factored_out

        elif isinstance(rhs, SeriesProduct) and isinstance(rhs.operands[-1], CPermutation):
            left_part =  cls.create(lhs, SeriesProduct.create(*rhs.operands[:-1],**rules),**rules)
            right_part = cls.create(cid(lhs.cdim), rhs.operands[-1], **rules)
            factored_out =  SeriesProduct.create(left_part, right_part, **rules)
            assert isinstance(factored_out, SeriesProduct)
            return factored_out
...
class CPermutation(Circuit, Expression):
    ...
    def factorize_for_rhs(self, rhs, **rules):
        ...
        permuted_rhs_circuit = Concatenation.create(*[SeriesProduct.create(within_blocks[p], rhs_blocks[p], **rules) \
...
class Feedback(Circuit, MultiaryOperation):
    ...
    @classmethod
    def create(cls, circuit, out_index = None, in_index = None, **rules):
        ...
        if isinstance(circuit, SeriesProduct): 
            ...
            if isinstance(lhs, CPermutation):
                ...
                return lhs_red << cls.create(SeriesProduct.create(*circuit.operands[1:], **rules), out_inv, in_index, **rules)
                ...
                if before != cid(nbefore) or after != cid(nafter):
                    ...
                    return outer_lhs << cls.create(SeriesProduct.create(inner_lhs, *circuit.operands[1:],**rules), out_index, in_index, **rules)
                elif block == cid(nblock):
                    outer_lhs = before + cid(nblock - 1) + after
                    return outer_lhs << cls.create(SeriesProduct.create(*circuit.operands[1:],**rules), out_index, in_index, **rules)
            ...
            if isinstance(rhs, CPermutation):
                ...
                return cls.create(SeriesProduct.create(*circuit.operands[:-1], **rules), out_index, in_im, **rules) << rhs_red
                ...
                if before != cid(nbefore) or after != cid(nafter):
                    ...
                    return cls.create(SeriesProduct.create(*(circuit.operands[:-1] + (inner_rhs,)),**rules), out_index, in_index, **rules) << outer_rhs
                elif block == cid(nblock):
                    outer_rhs = before + cid(nblock - 1) + after
                    return cls.create(SeriesProduct.create(*circuit.operands[:-1],**rules), out_index, in_index, **rules) << outer_rhs
...
class SeriesInverse(Circuit, UnaryOperation):
    ...
    @classmethod
    def create(cls, circuit, **rules):
        
        if isinstance(circuit, SeriesInverse):
            return circuit.operand
        elif isinstance(circuit, SeriesProduct):
            return SeriesProduct.create(*reversed(SeriesInverse.create(op, **rules) for op in circuit.operands), **rules)
        elif isinstance(circuit, Concatenation):
            return Concatenation.create(*[SeriesInverse.create(op, **rules) for op in circuit.operands], **rules)
        elif isinstance(circuit, Feedback):
            return Feedback(SeriesInverse.create(circuit.operand, **rules), *reversed(circuit.out_in_pair), **rules)
